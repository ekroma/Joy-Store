...
from apps.banners import BannerRead, BannerService, BannerDetail
...
router = APIRouter(
    prefix="/banner-client",
    tags=["Баннеры -> Клиентская часть"]
)


@router.get("", 
        response_model=PaginationBase[BannerDetail]|BannerDetail|None, 
...
async def get(
    ...
    if id:
        return await BannerService.get_by_id(id,market=market)
    return await BannerService.get_all(page, page_size,market=market)
